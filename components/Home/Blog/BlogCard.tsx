...
type blogProps = {
    blog: {
        ...
        date: string;
        image: string;
        url: string; // 1. Added url to type definition
    }
}

const BlogCard = ({blog}: blogProps) => {
    // 2. Destructure url here
    const {title, summary, date, image, url} = blog
  ...
  return (
    <div className='bg-indigo-950 rounded-md overflow-hidden'>
        ...
        <div className='p-6'>
            ...
            {/* Optional: You can also wrap the title in an <a> tag if you want the title to be clickable too */}
            ...
            <div className='flex items-center justify-between'>
                ...
                {/* 3. Changed button to <a> tag for navigation */}
                <a 
                    href={url}
                    target="_blank" 
                    rel="noopener noreferrer"
                    className='text-base hover:text-rose-500 underline text-gray-200 font-bold cursor-pointer'
                >
                    Read More
                </a>
            </div>
